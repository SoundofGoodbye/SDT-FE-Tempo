//delivery-system/apps/admin-panel/src/components/workflows/WorkflowCanvas.tsx
"use client";

import { WorkflowUI, WorkflowStepUI } from "@delivery-system/types";
import { Badge } from "@/components/ui/badge";
import { AlertTriangle, ArrowRight, X, Plus } from "lucide-react";
import { cn } from "@/lib/utils";

interface WorkflowCanvasProps {
  workflow: WorkflowUI | null;
  selectedStepId: number | null;
  onStepSelect: (stepId: number | null) => void;
  onStepDelete?: (stepId: number) => void;
  onStepAdd?: (afterStepId: number) => void;
  onStepReorder?: (stepIds: number[]) => void;
  className?: string;
}
...
export default function WorkflowCanvas({
  ...
  const handleStepDelete = (stepId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (onStepDelete) {
      onStepDelete(stepId);
    }
  ...
  const handleStepAdd = (afterStepId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (onStepAdd) {
  ...
  const getConnectionBottleneck = (fromStepId: number, toStepId: number) => {
    return workflow.connections?.find(
        (conn) => conn.fromStepId === fromStepId && conn.toStepId === toStepId,
    )?.isBottleneck;
  };

  // Check if this is a start or end step based on position
  const isStartStep = (step: WorkflowStepUI, index: number) => {
    return index === 0;
  };

  const isEndStep = (step: WorkflowStepUI, index: number) => {
    return index === workflow.steps.length - 1;
  };
        ...
        <div className="relative min-h-[400px] bg-gray-50 rounded-lg p-6">
          <div className="flex items-center justify-center space-x-2 flex-wrap">
            {workflow.steps.map((step, index) => {
              ...
              const isStart = isStartStep(step, index);
              const isEnd = isEndStep(step, index);
                  ...
                  <div key={step.id} className="flex items-center">
                    ...
                    >
                      <div
                          ...
                          style={{
                            borderColor: isSelected ? undefined : step.meta.color + "40",
                      ...
                      >
                        <Badge
                            className="mb-2"
                            style={{ backgroundColor: step.meta.color, color: "white" }}
                        >
                          {isStart ? "Start" : isEnd ? "End" : `${index}. ${step.customName}`}
                        </Badge>

                        {!isStartOrEnd && step.meta.stats && (
                            <div className="text-xs text-muted-foreground space-y-1">
                              <div>Avg: {step.meta.stats.avgTime}min</div>
                              <div>Success: {step.meta.stats.successRate}%</div>
                            </div>
                        )}
                    ...
                    {nextStep && (
                              ...
                              <span className="text-xs text-amber-600 ml-1">
                        {
                          ...
                          {/* Add step button */}
                          {onStepAdd && !isEnd && !isEndStep(nextStep, index + 1) && (
                              <button
